                ...
                let mut lines_vec = self
                    ...
                    .map(|msg| {
                        ...
                        (
                            {
                                let line_string =
                                    line.spans.iter().fold(String::new(), |mut acc, span| {
                                        acc.push_str(&span.content);
                                        acc
                                    });

                                let words = line_string.split_ascii_whitespace();
                                let bigger_than_line =
                                    words.clone().enumerate().filter(|(_idx, word)| {
                                        word.len() > (message_area.height as usize - 2)
                                    });
                                if bigger_than_line.clone().peekable().peek().is_none() {
                                    ...
                                    for (idx, word) in bigger_than_line {
                                        let lines_before = words_vec
                                            [(last_bigger_idx + 1) as usize..idx]
                                            .iter()
                                            .fold(0, |acc, word| acc + word.len() + 1)
                                            .div_ceil(message_area.width as usize - 2);
                                        lines_amount += lines_before;
                                        lines_amount +=
                                            word.len().div_ceil(message_area.width as usize - 2);
