...
pub fn run() {
    ...
    let mut conexao_servidor = TcpStream::connect(ip_servidor).expect("Não foi possível conectar ao servidor");
    conexao_servidor.write_all(meu_nome.as_bytes()).expect("Não foi possível enviar o nome de usuário ao servidor");
    ...
    loop {
        while let Ok(msg) = rec_msg.try_recv() {
            conexao_servidor.write_all(format!("{meu_nome}: {}", msg).as_bytes()).expect("Não consegui mandar sua mensagem");
        }
